...
class FobbuServiceAdapter(private var activity: Activity, private var dataListServices: ArrayList<HashMap<String, Any>> ): RecyclerView.Adapter<FobbuServiceAdapter.MyViewHolder>()
{
    ...
    {
        val view = LayoutInflater.from(activity!!).inflate(R.layout.rsa_services_adapter, p0, false)
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int)
    {
        ...
        if (dataListServices[position]["service_image"].toString() != "" || dataListServices[position]["service_image"]!= null)
            Picasso.get().load(dataListServices[position]["service_image"].toString()).error(R.drawable.dummy_services).into(holder.ivImage)
